...
def run_workflow(DATA_PATH):
    ...
    rc.add_replica("condorpool", data_loader_fn, os.path.join("file:///nfs/shared/panorama/bin/GalaxyClassification/", data_loader_fn))

    model_selction_fn = "model_selection.py"
    model_selction_file = File(model_selction_fn)
    rc.add_replica("condorpool", model_selction_fn, os.path.join("file:///nfs/shared/panorama/bin/GalaxyClassification/", model_selction_fn))

    preprocess_resize_fn = "preprocess_resize.py"
    preprocess_resize_file = File(preprocess_resize_fn)
    rc.add_replica("condorpool", preprocess_resize_fn, os.path.join("file:///nfs/shared/panorama/bin/GalaxyClassification/", preprocess_resize_fn))

    preprocess_augment_fn = "preprocess_augment.py"
    preprocess_augment_file = File(preprocess_augment_fn)
    rc.add_replica("condorpool", preprocess_augment_fn, os.path.join("file:///nfs/shared/panorama/bin/GalaxyClassification/", preprocess_augment_fn))

    vgg16_hpo_fn = "vgg16_hpo.py"
    vgg16_hpo_file = File(vgg16_hpo_fn)
    rc.add_replica("condorpool", vgg16_hpo_fn, os.path.join("file:///nfs/shared/panorama/bin/GalaxyClassification/", vgg16_hpo_fn))

    train_model_vgg16_fn = "train_model_vgg16.py"
    train_model_vgg16_file = File(train_model_vgg16_fn)
    rc.add_replica("condorpool", train_model_vgg16_fn, os.path.join("file:///nfs/shared/panorama/bin/GalaxyClassification/", train_model_vgg16_fn))
    
    eval_model_vgg16_fn = "eval_model_vgg16.py"
    eval_model_vgg16_file = File(eval_model_vgg16_fn)
    rc.add_replica("condorpool", eval_model_vgg16_fn, os.path.join("file:///nfs/shared/panorama/bin/GalaxyClassification/", eval_model_vgg16_fn))
    ...
    rc.add_replica("condorpool", vgg16_pkl, os.path.join("file:///nfs/shared/panorama/data/GalaxyClassification/config/", vgg16_pkl))    

    # FILES FOR train_model.py 
    #--------------------------------------------------------------------------------------------------------
    checkpoint_vgg16_pkl = "checkpoint_vgg16.pkl"
    checkpoint_vgg16_pkl_file = File(checkpoint_vgg16_pkl)
    rc.add_replica("condorpool", checkpoint_vgg16_pkl, os.path.join("file:///nfs/shared/panorama/data/GalaxyClassification/config/", checkpoint_vgg16_pkl))
    ...
    galaxy_container = Container(
                ...
                image = "file:///nfs/shared/panorama/containers/galaxy-classification_latest.sif",
    ...
    preprocess_images = Transformation("preprocess_images", site="condorpool",
                                    pfn = "file:///nfs/shared/panorama/bin/GalaxyClassification/darshan_wrapper.sh",
                                    is_stageable= False, container=galaxy_container)
    ...
    for j in job_preprocess_images:
        j.add_inputs(preprocess_resize_file)
        j.add_args(preprocess_resize_fn)
    ...
    job_augment_class_2 = Job(augment_images)\
                        .add_args(preprocess_augment_fn, "--class_str class_2 --num {}".format(NUM_CLASS_2))\
                        .add_inputs(preprocess_augment_file, *input_aug_class_2)\
                        .add_outputs(*output_aug_class_2)

    job_augment_class_3 = Job(augment_images)\
                        .add_args(preprocess_augment_fn, "--class_str class_3 --num {}".format(NUM_CLASS_3))\
                        .add_inputs(preprocess_augment_file, *input_aug_class_3)\
    ...
    job_vgg16_hpo = Job(vgg16_hpo)\
                        .add_args(vgg16_hpo_fn, "--trials {} --epochs {} --batch_size {}".format(TRIALS, EPOCHS, BATCH_SIZE))\
                        .add_inputs(vgg16_hpo_file, *output_aug_class_3, *output_aug_class_2,\
    ...
    job_train_model = Job(train_model)\
                        .add_args(train_model_vgg16_fn, "--epochs {} --batch_size {}".format( EPOCHS, BATCH_SIZE))\
                        .add_inputs(train_model_vgg16_file, *output_aug_class_3, *output_aug_class_2, best_params_file,\
    ...
    job_eval_model = Job(eval_model)\
                        .add_args(eval_model_vgg16_fn)\
                        .add_inputs(eval_model_vgg16_file, *input_hpo_test,data_loader_file,best_params_file,\
...
def main():
    ...
    DATA_PATH = "file:///nfs/shared/panorama/data/GalaxyClassification"
